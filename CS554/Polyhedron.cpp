...
int Polyhedron::add_vertex(Vertex* vert) {
	return -1;
}

int Polyhedron::del_edge(Edge* edge) {
	return -1;
}

int Polyhedron::del_tris(Triangle* tris) {
	return -1;
}
...
void Polyhedron::vertex_to_tri_ptrs()
{
	...
	for (i = 0; i < nverts; i++) {
		...
		vlist[i]->tris = new Triangle*[vlist[i]->max_tris];
		vlist[i]->ntris = 0;
	}
...
void Polyhedron::print_vertices() {
	printf("#Vertices:%d\n",nverts);
	for (int i = 0; i < nverts; i++) {
		Vertex* v = vlist[i];
		printf("%4d: %5.4f,%5.4f,%5.4f\n",v->index,v->x,v->y,v->z);
	}
}
